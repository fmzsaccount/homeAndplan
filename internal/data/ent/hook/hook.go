...
// The PlanCategoryFunc type is an adapter to allow the use of ordinary
// function as PlanCategory mutator.
type PlanCategoryFunc func(context.Context, *ent.PlanCategoryMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PlanCategoryFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.PlanCategoryMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PlanCategoryMutation", m)
	}
	return f(ctx, mv)
}
